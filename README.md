# LW008_MTE Android SDK Guide（English）
...
**`com.moko.support.lw008mte.LoRaLW008MTEMokoSupport`**
...
**`com.moko.support.lw008mte.OrderTaskAssembler`**

We assemble read data and write data to `OrderTask`, send the task to the device
through `LoRaLW008MTEMokoSupport `, and receive the resopnse.
...
First of all, you should initialize the LoRaLW008MTEMokoSupport.We recommend putting it in Application.
...
in LoRaLW008MTEMokoSupport `AndroidManifest.xml`
...
```
LoRaLW008MTEMokoSupport.getInstance().connDevice(beaconXInfo.mac);
```
...
```
LoRaLW008MTEMokoSupport.getInstance().sendOrder(OrderTaskAssembler.setPassword(password));
...
```
// read lora region
LoRaLW008MTEMokoSupport.getInstance().sendOrder(derTaskAssembler.getLoraRegion());
...
// get result
@Subscribe(threadMode = ThreadMode.MAIN)
public void onOrderTaskResponseEvent(OrderTaskResponseEvent event) {
...
2.`EventBus` is used in the SDK and can be modified in `LoRaLW008MTEMokoSupport` if you want to use
